...
# ============================================================
# Вспомогательные функции (Safety)
# ============================================================

# Функция безопасного выхода.
# Спасает терминал от закрытия при запуске через source
safe_exit() {
    local code=${1:-1}
...
show_help() {
    echo -e "${BLUE}Android Build Wrapper (v2.0)${NC}"
...
run_env_setup() {
    ...
    _install_fedora() {
        echo -e "${YELLOW}[DNF] Обнаружена Fedora ($OS_ID).${NC}"
        local DNF_CMD="dnf"
        if command -v dnf5 &> /dev/null; then DNF_CMD="dnf5"; fi

        if grep -q "exclude=.*i686" /etc/dnf/dnf.conf; then
            sed -i 's/^exclude=/#exclude=/g' /etc/dnf/dnf.conf
        fi

        $DNF_CMD upgrade --refresh -y
        $DNF_CMD install -y --skip-broken @"Development Tools" @"C Development Tools and Libraries" || true
        ...
        $DNF_CMD install -y --skip-broken $PKGS
        $DNF_CMD install -y java-21-openjdk-devel || $DNF_CMD install -y java-17-openjdk-devel

        if [ ! -f /usr/lib64/libncurses.so.5 ]; then
            ln -s /usr/lib64/libncurses.so.6 /usr/lib64/libncurses.so.5 || true
            ln -s /usr/lib64/libtinfo.so.6 /usr/lib64/libtinfo.so.5 || true
        fi
        ccache -M 80G
    }
...
main() {
      ...
      case $1 in
		...
		--kernel)
			...
			if [ -z "$2" ]; then
				...
				return 1 # Или exit 1, в зависимости от контекста
			fi
    ...
    if [ "$DO_BUILD" = true ]; then
        ...
        # Lunch
        if [ "$FORCE_LUNCH" = true ] || [[ "$TARGET_PRODUCT" != *"$DEVICE_CODE"* ]]; then
